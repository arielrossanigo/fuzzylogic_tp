...
class Pendulum(object):
    ...
    #: lower limit of velocity. Smaller values of velocity are
    #considered as no movement.
    NO_MOVEMENT = 0.00001
    ...
    def __init__(self):
        ...
        self.Phi = math.radians(89.0)  # angle [rad]
        ...
        self.M_P = 0.1  # friction of bearing of pendulum expressed
                        # as torque [kgm²/s²=Nm]
    ...
    def doSmallStep(self, dT):
        ...
        while dT > 0.0:
            ...
            if (self.car_moves == 0) and (abs(a) > self.a_W):
                self.car_moves = 1
                if a > 0:
                    self.dX_dT = + Pendulum.NO_MOVEMENT
                else:
                    self.dX_dT = -Pendulum.NO_MOVEMENT

            # XXX Reibung zu gross => Stop
            if ((abs(self.dX_dT) < Pendulum.NO_MOVEMENT) and
               (abs(a) <= self.a_W)):
                self.car_moves = 0
                self.dX_dT = 0.0
                a = 0.0
            ...
            if self.car_moves == 1:
                ...
                if self.dX_dT * (self.dX_dT + a * dT) < 0.0:
                    # Zeiteinheit teilen
                    dT_ = - self.dX_dT / a
            ...
            while dT > 0.0:
                ...
                M_D = self.l / 2.0 * self.m * (
                      ...
                      + (self.Z * random.gauss(0.0, 1.0)) * math.sin(self.Phi)
                      )
                ...
                if (self.pendulum_moves == 0) and (abs(M_D) > self.M_P):
                    ...
                    if M_D > 0.0:
                        self.dPhi_dT = +Pendulum.NO_MOVEMENT
                    else:
                        self.dPhi_dT = -Pendulum.NO_MOVEMENT
                if (abs(self.dPhi_dT) < Pendulum.NO_MOVEMENT) and \
                   (abs(M_D) <= self.M_P):
                ...
                if self.pendulum_moves:
                    ...
                    d2Phi_dT2 = M_D / (4.0 / 3.0 * self.m * self.l *  \
                                self.l / 4.0)  # inertia

                    if self.dPhi_dT * (self.dPhi_dT + d2Phi_dT2 * dT) < 0.0:
                        # Zeiteinheit teilen
